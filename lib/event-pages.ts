import { getAlerts, getCaseRecords, getSourcesByIds } from "@/lib/data";
import { SITE_URL, absoluteUrl } from "@/lib/routes";

export type EventPageRecord = {
  id: string;
  type: "official-alert" | "case-summary";
  title: string;
  subtitle: string;
  date: string;
  geography: string;
  status: string;
  summary: string;
  riskLanguage: string;
  sourceIds: string[];
  canonicalPath: string;
  rawPath: string;
  metrics: Array<{ label: string; value: string }>;
};

export const EVENT_PAGE_IDS = [
  "who-don600-2026-cruise",
  "canada-mv-hondius-2026",
  "ecdc-mv-hondius-response-2026",
  "paho-alert-2025-americas",
  "nmdoh-2026-santa-fe",
  "adhs-2024-increased-activity",
  "utah-mv-hondius-2026",
  "united-states-cdc-1993-2023",
  "canada-phac-1994-2026",
  "argentina-ben-source-linked-2025",
] as const;

export function eventPath(id: string) {
  return `/event/${id}/`;
}

export function rawEventPath(id: string) {
  return `/raw/event/${id}/`;
}

export function getEventRecords(): EventPageRecord[] {
  const alertRecords = getAlerts().map((alert): EventPageRecord => ({
    id: alert.id,
    type: "official-alert",
    title: alert.title,
    subtitle: `${alert.agency} · ${alert.severityLabel}`,
    date: alert.date,
    geography: alert.geography,
    status: alert.status,
    summary: alert.summary,
    riskLanguage: alert.riskLanguage,
    sourceIds: alert.sourceIds,
    canonicalPath: eventPath(alert.id),
    rawPath: rawEventPath(alert.id),
    metrics: [
      { label: "Layer", value: alert.layer },
      { label: "Agency", value: alert.agency },
      { label: "Status", value: alert.status },
    ],
  }));

  const caseRecords = getCaseRecords().map((record): EventPageRecord => ({
    id: record.id,
    type: "case-summary",
    title: `${record.jurisdiction} hantavirus source-linked case summary`,
    subtitle: record.reportLabel,
    date: record.period,
    geography: `${record.jurisdiction} / ${record.region}`,
    status: record.status,
    summary: record.summary,
    riskLanguage: record.limitations.join(" "),
    sourceIds: record.sourceIds,
    canonicalPath: eventPath(record.id),
    rawPath: rawEventPath(record.id),
    metrics: [
      { label: "Period", value: record.period },
      { label: "Reported cases", value: record.reportedCases === null ? "Linked official source" : String(record.reportedCases) },
      { label: "Precision", value: record.geographyPrecision },
    ],
  }));

  const events = [...alertRecords, ...caseRecords];
  return events.filter((event) => EVENT_PAGE_IDS.includes(event.id as (typeof EVENT_PAGE_IDS)[number]));
}

export function getEventRecord(id: string) {
  return getEventRecords().find((event) => event.id === id);
}

function escapeYaml(value: string) {
  return JSON.stringify(value);
}

export function renderEventMarkdown(event: EventPageRecord) {
  const sources = getSourcesByIds(event.sourceIds);
  const sourceLines = sources
    .map(
      (source) =>
        `- ${source.publisher}: [${source.title}](${source.url}) — reviewed ${source.reviewedDate}; use: ${source.use}`,
    )
    .join("\n");
  const metricLines = event.metrics.map((metric) => `| ${metric.label} | ${metric.value} |`).join("\n");

  return `---
id: ${escapeYaml(event.id)}
type: ${escapeYaml(event.type)}
title: ${escapeYaml(event.title)}
geography: ${escapeYaml(event.geography)}
date: ${escapeYaml(event.date)}
status: ${escapeYaml(event.status)}
canonical: ${escapeYaml(absoluteUrl(event.canonicalPath))}
license: "CC-BY-4.0"
---

# ${event.title}

${event.subtitle}

## Summary

${event.summary}

## Safe interpretation

${event.riskLanguage}

This record is a reviewed public health map note. It is not live surveillance, medical advice, patient-location tracking, or county-level risk prediction.

## Key fields

| Field | Value |
|---|---|
| Geography | ${event.geography} |
| Date / period | ${event.date} |
| Status | ${event.status} |
${metricLines}

## Sources

${sourceLines}

## Cite this record

${absoluteUrl(event.canonicalPath)} (accessed YYYY-MM-DD)

Dataset: ${SITE_URL}/data/map-snapshots.json
`;
}
